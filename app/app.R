...
#Read sunburst data
sun_data <- fread("Sunburst_data.csv", sep =";", sep2 = ",", colClasses = list("character" = 1))

#Read collection data
data.types <- fread("Expandedcollectiondata.csv", nrows = 1, header = TRUE) %>% colnames()
...
var.legend <- (fread("Expandedcollectiondata.csv", nrows = 3, drop = c(1,2), data.table = F) %>% t())[,c(3,2)]
...
super.groups <- (sun_data[grep(sun_data$ParentCNR, pattern="[A-Z][a-z]"),2] %>% unique())$ParentCNR

descripcionED <- c("Necesita prueba de germinacion"	,
                   "Disponible"	,
                   "Duplicado"	,
  ...
  tabItems(
    tabItem("sunburst",
            fluidRow(
              ...
              box( # DISPLAY SUNBURST
                
                title = p(div(style="display: inline-block;vertical-align:top; width: 175px;","Select group of interest"),
                          div(style="display: inline-block;vertical-align:top; width: 5px;"),
                          div(style="display: inline-block;vertical-align:top; width: 50px;",
                              actionButton("root", "", icon = icon("undo", lib = "font-awesome" , verify_fa = FALSE),
                                           class = "btn-xs", title = "Back to start"))
                ),
                div(style="display:inline-block; vertical-align:top; width: 200px;",
                    selectInput("sunburst_var", "Choose a variable:", 
                ...
                div(style="display:inline-block; vertical-align:top; width: 70px;", 
                    numericInput("sun_depth", "Plot depth:", min = 1, max = 4, step = 1, value = 2.0
                    )),
              ...
              box( # DISPLAY VARIABLE DESCRIPTIONS
                
                dataTableOutput("var_legend_table"),
                status = "success",
                title = "Descripcion de variable",
                collapsible = TRUE, collapsed = TRUE, width = 6), #collapsed determines whether the box is collapsed at startup
              ...
              box( # DISPLAY VARIABLE DESCRIPTIONS
                ...
                collapsible = TRUE, collapsed = TRUE, width = 6,  height = "auto"),
              
              tabBox(
                
                tabPanel( # DISPLAY HISTOGRAM
                  
                  title = "Histogram analysis",
                  div(style="display:inline-block; vertical-align:top; width: 150px;", 
                      selectInput("hist_var", "Choose a variable:",
                  ...
                  div(style="display: inline-block;vertical-align:top; width: 70px;", 
                      numericInput("bin_size", "Bin size:", min = 0.2, step = 0.1, value = 1.0
                      )),
                  textOutput("pathText1"),
                  plotlyOutput("var_hist"),
                  width = 6),
                
                tabPanel( # DISPLAY GERMINATION SCATTERPLOT
              ...
              box( # DISPLAY TABLE
                ...
                title = p(div(style="display: inline-block;vertical-align:top; width: 150px;","Selected accessions"),
                          div(style="display: inline-block;vertical-align:top; width: 5px;"),
                          div(style="display: inline-block;vertical-align:top; width: 50px;",
                              actionButton("remove", "", icon = icon("undo", lib = "font-awesome", verify_fa = FALSE),
                                           class = "btn-xs", title = "Undo selection", verify_fa = FALSE))
                ),
    ...
    tabItem("download_page",
            fluidPage(
              box(title = "Export your custom selection",
...
server <- function(input, output) {
  ...
  selection_modal <- modalDialog(title = "You created a selection", 
                                 "Apply selection to datatable?", 
                                 footer =  tagList(actionButton("apply", label = "Apply"), 
                                                   modalButton("Cancel")), 
  )
  
  multiple_sel_modal <- modalDialog(title = "Another selection is active",
                                    "You created a selection while another selection is active. What do you want to do?",
                                    footer = tagList(actionButton("combine", label = "Combine"),
  ...
  sunburst <- reactiveValues(ids = sun_data$CNR, labels = sun_data$Cat, 
                             parents = sun_data$ParentCNR, values = sun_data$Count,
                             var = sun_data$`Clases de gerninacion` , path = "root", na.count = 0)
  ...
  sunSelection <- reactive({
    ...
    if (sunburst$path == "root") {
      ...
      paths <- sun_data %>% filter(ParentCNR == sunburst$path) %>% pull(CNR)
  ...
  datatable <- reactiveValues(selection = data, selection_counter = 0, persistence = F)
  ...
  observeEvent(input$intersect, {
    ...
    datatable$selection <- intersect(datatable$selection, plot$selected)
    removeModal()
  }) 
  ...
  observeEvent(input$sunburst_var, priority = 10, {
    sunburst$na.count <- sun_data[[paste(input$sunburst_var, "NA", sep = ".")]]
  ...
  observeEvent(event_data(event = "plotly_sunburstclick",
                          source = "sunSource", priority = "event"), {
                            clickdata <- unlist(event_data(event = "plotly_sunburstclick",
                                                           source = "sunSource", priority = "event"))[["customdata"]] %>% as.character()
                            if(clickdata != sunburst$path){
                              ...
                              sunburst$path <- sun_data$ParentCNR[which(sun_data$CNR == sunburst$path)]
                            }
  ...
  observeEvent(event_data(event = "plotly_selected",
                          source = "histSource", priority = "event"), {
                            hist$xselect <- event_data(event = "plotly_selected",
                                                       source = "histSource", priority = "event")[["x"]]
                            plot$selected <- sunSelection()[inrange(sunSelection()[[input$hist_var]], lower = (hist$xselect - 0.5), upper = (hist$xselect + 0.5)),]
                            if(datatable$selection_counter == 0){
  ...
  #Retrieve selection data from histogram
  observeEvent(event_data(event = "plotly_selected",
                          source = "scatterSource", priority = "event"), {
                            scatter$pointselect <- event_data(event = "plotly_selected", source = "scatterSource", priority = "event")[["pointNumber"]]
                            plot$selected <- sunSelection()[scatter$pointselect+1,]
                            if(datatable$selection_counter == 0){
                              showModal(selection_modal)}
                            else if(datatable$selection_counter >0){
                              showModal(multiple_sel_modal)
  ...
  output$var_legend_table <- renderDataTable({
    ...
    datatable(var.legend, rownames = FALSE, options = list(dom = "t"),
              selection = "none")
  })
  
  output$germination_description <- renderDataTable({
    #Descripcion Germinatio Class
    datatable(VrDs, rownames = FALSE, options = list(dom = 't', pageLength = 25),
              selection = "none")
  ...
  output$show_selection <- renderDataTable(
    ...
    datatable$selection %>%
      ...
      datatable(filter = "top", extensions = 'Buttons', 
                options = list(pageLength = 50, columns.width = 50, dom = "Btip",
                               buttons = list("pageLength", "colvis", 
                                              list(
                                                extend = 'collection',
  ...
  output$sunburst_plot <- renderPlotly({
    sun_data %>%
      plot_ly( 
        ...
        marker = list(colors = sunburst$var, 
                      showscale = TRUE,
                      colorscale = "Greens", reversescale = TRUE,
                      colorbar = list(lenmode = "fraction", len = 0.5,
                      ...
                      cmin = 0, cmax = (quantile(sunburst$var, probs = 0.95, na.rm = T)) %>% round(0)
        ),
        level = sunburst$centre,
        hoverinfo = "text",
        hovertext = ~paste(sunburst$labels, "<br>",
                           "Number of accessions: ", sunburst$values, "<br>",
                           input$sunburst_var, ": ", sunburst$var, sep = "")
      ) %>%
      config(modeBarButtonsToRemove = c("toggleHover", "hoverCompareCartesian"),
             displaylogo = FALSE) %>%
      layout(
        # annotations = list(text = "Excluding accessions and groups without data",
  ...
  output$germ_scatter <- renderPlotly(
    {
      ggplotly(
        ...
        (sunSelection() %>%
           ...
           ggplot(aes(x = `Estado de disponibilidad`, y = `Fecha de ultima prueba`, text = ARN)) +
           geom_jitter(alpha = 0.15, shape = 1, colour = "#2ca02c", width = 0.3) +
           labs(x = "Estado de disponbilidad", y = "Fechas de ultima prueba (Anios)") +
           theme_minimal()), 
        tooltip = "text"
      ) %>%
        layout(dragmode = "select") %>%
        config(modeBarButtonsToRemove = c("pan2d", "zoom2d", "zoomIn2d", "zoomOut2d",
  ...
  output$var_hist <- renderPlotly(
    {
      sunSelection() %>%
        ...
        layout(xaxis = list(range = c(-0.5*input$bin_size, (max(data[[input$hist_var]], na.rm = TRUE)+0.5*input$bin_size)),
                            title = input$hist_var,
                            tick0 = 0, dtick = 2*input$bin_size,
                            type = hist$layout.type),
               yaxis = list(title = "Count", fixedrange = TRUE),
               dragmode = "select"
        ) %>%
        config(modeBarButtonsToRemove = c("pan2d", "lasso2d", "zoomIn2d", "zoomOut2d",
                                          "hoverClosestCartesian", "hoverCompareCartesian",
                                          "toggleSpikelines", "autoScale2d"),
               displaylogo = FALSE)
    })
